      ...
      ({handleDelete, handleEdit, handleFavorite}) => {
        return (
          <tr className="activity-run">
            <td className="data-date"><span className="value">{Option(event.date).map(formatDate).orElse('-')}</span></td>
            ...
            <td className="data-category"><span className="value">{Option(event.category).orElse('-')}</span></td>
            <td className="data-distance"><span className="value">{Option(event.distance).map(d => d + ' mi').orElse('-')}</span></td>
            <td className="data-duration"><span className="value">{Option(event.duration).map(formatDuration).orElse('-')}</span></td>
            <td className="data-notes"><span className="value">{Option(event.notes).orElse('-')}</span></td>
